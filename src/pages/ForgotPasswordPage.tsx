...
const ForgotPasswordPage = () => {
  ...
  const isVendor = location.pathname.startsWith('/vendor/forgot-password');
  const isHost = location.pathname.startsWith('/host/forgot-password');
  
  // Determine the appropriate login path based on route
  const getLoginPath = () => {
    if (isVendor) return '/vendor/login';
    if (isHost) return '/host/login';
    // Default fallback
    return '/vendor/login';
  };
  
  // Determine the role to send to API
  const getRole = () => {
  ...
  const onSubmit = async (data: EmailFormData) => {
    ...
    try {
      ...
      setEmailSent(true);
      toast.success(response.message || 'If an account exists with this email, you will receive a password reset link.');
    } catch (error: any) {
              ...
              <div className="space-y-2">
                ...
                <Button 
                  onClick={() => {
                    ...
                    console.log('Forgot password - back to login redirecting to:', loginPath);
                    navigate(loginPath);
                  }}
                  className="w-full"
                >
