...
function AdminOrderSummaryPage() {
  ...
  useEffect(() => {
    ...
    console.log(
      "[AdminOrderSummaryPage] location.pathname:",
      location.pathname,
    );
  ...
  useEffect(() => {
    const loadInvoiceSummary = async () => {
      if (!invoiceIdFromUrl) {
        console.log(
          "[AdminOrderSummaryPage] No invoiceId in URL, skipping API call",
        );
        return;
      }

      console.log(
        "[AdminOrderSummaryPage] Loading invoice summary from API for invoiceId:",
        invoiceIdFromUrl,
      );
      setIsLoadingInvoiceSummary(true);
      try {
        const response =
          await invoiceService.getInvoiceOrderSummary(invoiceIdFromUrl);
        ...
        if (response?.data?.invoice) {
          ...
          const mappedServices: ServiceSelection[] = (
            ...
            if (service.cateringItems && service.cateringItems.length > 0) {
              ...
              menuItems = service.cateringItems.map((item: any) => {
                const comboCategoryItems = (invoiceData.services || []).flatMap(
                  (s: any) =>
          ...
          const allComboCategoryItems = (invoiceData.services || []).flatMap(
                  ...
                  <PaymentMethodCard
                    ...
                    preTaxBreakdown={{
                      ...
                      deliveryFee: selectedServices.reduce((total, service) => {
                        const deliveryFee =
                          parseFloat((service as any).deliveryFee || "0") || 0;
                        return total + deliveryFee;
                      }, 0),
                      adjustmentsTotal:
