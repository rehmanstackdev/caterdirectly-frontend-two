...
function OrderDetailsPage() {
    ...
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        if (response?.data?.invoice) {
          ...
          // Include ALL services (no filtering by vendor)
          const allServices = invoiceData.services || [];

          const mappedSelectedItems: Record<string, number> = {};

          const mappedServices = allServices.map((service: any) => {
            ...
            if (service.cateringItems && service.cateringItems.length > 0) {
              menuItems = service.cateringItems.map((item: any) => {
                ...
                const comboCategoryItems = allServices.flatMap((s: any) => 
          ...
          setOrderData({
            ...
            waiveServiceFee: invoiceData.waiveServiceFee || false,
    ...
    return (
      <Dashboard activeTab="analytics" userRole="event-host">
        <div className="text-center py-8">
          ...
          <Button onClick={() => navigate('/host/analytics?section=orders')} className="mt-4">
            Back to Orders
          </Button>
        </div>
      </Dashboard>
  ...
  const calculateCustomLineItems = () => {
    ...
    return { totalDiscounts, totalSurcharges };
  };

  const calculateServiceFee = () => {
    if (orderData.waiveServiceFee) return 0;
    const subtotal = calculateSubtotal();
    return subtotal * 0.05; // 5% service fee
  };

  const calculateGrandTotal = () => {
    const { totalDiscounts, totalSurcharges } = calculateCustomLineItems();
    return calculateSubtotal() + totalSurcharges - totalDiscounts + calculateServiceFee();
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate('/host/analytics?section=orders')}
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {orderData.services.map((service: any) => {
                  ...
                  <React.Fragment key={service.id}>
                    <TableRow className="bg-[#F07712]/10 border-l-4 border-[#F07712]">
                      <TableCell className="font-semibold" colSpan={service.serviceType === 'catering' ? 5 : 1}>
                      ...
                      {service.serviceType !== 'catering' && (
                        <>
                          <TableCell>
                            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-[#F07712]/20 text-[#F07712]">
                              {serviceTypeLabel}
                            </span>
                          </TableCell>
                          ...
                          <TableCell className="pl-6">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="text-xs text-muted-foreground">Combo Package</div>
                              </div>
                            </div>
                          </TableCell>
                          <TableCell>
                              ...
                              <TableCell className="pl-12">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <div className="text-xs text-muted-foreground">from {comboItem.menuName}</div>
                                  </div>
                                </div>
                              </TableCell>
                              <TableCell>
                    ...
                    <TableRow className="bg-muted/20">
                      <TableCell colSpan={4} className="text-right font-semibold">Service Total:</TableCell>
                ...
                {calculateCustomLineItems().totalDiscounts > 0 && (
                  <TableRow className="bg-muted/20">
                    <TableCell colSpan={4} className="text-right font-semibold">Discounts:</TableCell>
                    <TableCell className="text-right font-semibold text-green-600">
                      -${calculateCustomLineItems().totalDiscounts.toFixed(2)}
                    </TableCell>
                  </TableRow>
                )}
                {calculateCustomLineItems().totalSurcharges > 0 && (
                  <TableRow className="bg-muted/20">
                    <TableCell colSpan={4} className="text-right font-semibold">Surcharges:</TableCell>
                    <TableCell className="text-right font-semibold">
                      ${calculateCustomLineItems().totalSurcharges.toFixed(2)}
                    </TableCell>
                  </TableRow>
                )}
                <TableRow className="bg-muted/20">
                  <TableCell colSpan={4} className="text-right font-semibold">Service Fee (5%):</TableCell>
                  <TableCell className="text-right font-semibold">
                    ${calculateServiceFee().toFixed(2)}
                  </TableCell>
                </TableRow>
                <TableRow className="bg-muted/20">
                  <TableCell colSpan={4} className="text-right font-semibold">Delivery Fee:</TableCell>
                  <TableCell className="text-right font-semibold">
                    $0.00
                  </TableCell>
                </TableRow>
