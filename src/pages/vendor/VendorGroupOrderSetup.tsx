...
function VendorGroupOrderSetup() {
  ...
  useEffect(() => {
    ...
    if (changingService && incomingService && typeof serviceIndex === 'number' && currentServices) {
      console.log('[VendorGroupOrderSetup] Replacing service at index:', serviceIndex);
      ...
      if (updatedServices[serviceIndex]) {
        ...
        console.log('[VendorGroupOrderSetup] Service replaced successfully:', newSelection.serviceName || newSelection.name);
      }
    ...
    // Case 2: If returning from marketplace with cart items, merge them with existing services
      ...
      console.log(
        "[VendorGroupOrderSetup] Merging cart items with existing services"
      );
    ...
    // Set order type to Group Order if returning from marketplace with group order flag
    // or if this is a group order setup page (should always be true for this page)
    if (isGroupOrderFromState !== undefined) {
      setOrderType(isGroupOrderFromState);
    } else {
      // Default to true for group order setup page
      setOrderType(true);
    }
          ...
          if (matchingRange) {
            setServiceDeliveryFees((prev) => {
              ...
              try {
                ...
                console.warn('[VendorGroupOrderSetup] Failed to persist delivery fees:', error);
              }
              return updated;
            });
          }
  ...
  const handleLocationSelected = useCallback((address: string, locationData?: LocationData) => {
    if (locationData) {
      ...
      try {
        ...
        console.warn('[VendorGroupOrderSetup] Failed to persist location data:', error);
      }
      console.log('[VendorGroupOrderSetup] Location selected with coordinates:', locationData);
    }
  }, []);
  ...
  const handleRemoveService = useCallback((serviceIndex: number) => {
    return () => {
      ...
      console.log('[VendorGroupOrderSetup] Service removed:', serviceName);
    };
    ...
    for (const service of state.selectedServices) {
      ...
      if (serviceType === "catering") {
        ...
        if (minimumOrderAmount > 0 && details) {
          ...
          console.log('[VendorGroupOrderSetup] Minimum order amount validation:', {
            serviceName,
            serviceTotal,
            minimumOrderAmount,
            guestCount,
    ...
    try {
      ...
      const mapServiceWithItems = (service: any, index: number) => {
          ...
          service.comboSelectionsList.forEach((combo: any) => {
            if (combo) {
              ...
              if (import.meta.env.DEV) {
                console.log('[VendorGroupOrderSetup] Combo pricing calculation:', {
        ...
        if (normalizedServiceType === "catering") {
          ...
          if (import.meta.env.DEV) {
            console.log('[VendorGroupOrderSetup] Delivery ranges for service:', {
              serviceName: service.serviceName || service.name,
              deliveryOptions,
          ...
          if (service.service_details) {
            ...
            if (import.meta.env.DEV) {
              console.log('[VendorGroupOrderSetup] Catering service total calculation:', {
                serviceName: service.serviceName || service.name,
                basePricePerPerson,
                guestCount,
     ...
     // console.log(JSON.stringify(invoiceData))
      ...
      try {
        ...
        console.warn('[VendorGroupOrderSetup] Failed to clear localStorage:', error);
      }
      toast.success(
...
export default VendorGroupOrderSetup;
