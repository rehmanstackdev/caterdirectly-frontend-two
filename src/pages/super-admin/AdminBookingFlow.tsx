...
function VendorBookingFlow() {
  ...
  useEffect(() => {
    if (eventLocationData && formData.location) {
      console.log('[AdminBookingFlow] Location data restored from localStorage:', {
  ...
  useEffect(() => {
    ...
    if (previousLocationRef.current && currentLocation && previousLocationRef.current !== currentLocation) {
      console.log('[AdminBookingFlow] Location changed, clearing delivery fees for recalculation');
      setServiceDeliveryFees({});
      try {
        localStorage.removeItem('serviceDeliveryFees');
      } catch (error) {
        console.warn('[AdminBookingFlow] Failed to clear delivery fees from localStorage:', error);
      }
    }
  ...
  useEffect(() => {
    ...
    if (Object.keys(distancesByService).length === 0 || !formData.location) {
      console.log('[AdminBookingFlow] Skipping auto-calculation: no distances or location');
      return;
    }
    ...
    selectedServices.forEach((service) => {
      ...
      if (!distance || distance <= 0) {
        console.log(`[AdminBookingFlow] Skipping service ${serviceId}: no valid distance`);
        return;
      }
      ...
      if (serviceDeliveryFeesRef.current[serviceId]) {
        console.log(`[AdminBookingFlow] Skipping service ${serviceId}: fee already exists`);
        return;
      }
      ...
      if (deliveryOptions?.delivery && deliveryOptions.deliveryRanges) {
        ...
        console.log(`[AdminBookingFlow] Delivery calculation result for ${serviceId}:`, deliveryResult);
        ...
        if (deliveryResult.eligible && deliveryResult.fee >= 0) {
          ...
          if (matchingRange) {
            console.log(`[AdminBookingFlow] Auto-applying delivery fee for ${serviceId}:`, matchingRange);
            newFees[serviceId] = matchingRange;
          } else {
            console.warn(`[AdminBookingFlow] No matching range found for ${serviceId}`);
          }
        } else {
          console.log(`[AdminBookingFlow] Delivery not eligible for ${serviceId}`);
        }
      } else {
        console.log(`[AdminBookingFlow] No delivery options for service ${serviceId}`);
      }
    });
    ...
    if (Object.keys(newFees).length > 0) {
      ...
      setServiceDeliveryFees((prev) => {
        ...
        try {
          localStorage.setItem('serviceDeliveryFees', JSON.stringify(updated));
          console.log('[AdminBookingFlow] Persisted delivery fees to localStorage');
        } catch (error) {
          console.warn('[AdminBookingFlow] Failed to persist delivery fees:', error);
        }
        return updated;
      });
    } else {
  ...
  const handleLocationSelected = useCallback((address: string, locationData?: LocationData) => {
    if (locationData) {
      ...
      try {
        ...
        console.warn('[AdminBookingFlow] Failed to persist location data:', error);
      }
      console.log('[AdminBookingFlow] Location selected with coordinates:', locationData);
    }
  }, []);
