...
function BookingFlow() {
  ...
  // Memoize destinationCoordinates to prevent infinite re-renders
  const destinationCoordinates = useMemo(() => {
    if (!eventLocationData?.coordinates) return null;
    return { lat: eventLocationData.coordinates.lat, lng: eventLocationData.coordinates.lng };
  }, [eventLocationData?.coordinates?.lat, eventLocationData?.coordinates?.lng]);
          ...
          if (matchingRange) {
            setServiceDeliveryFees((prev) => {
              ...
              try {
                ...
                console.warn('[BookingFlow] Failed to persist delivery fees:', error);
              }
              return updated;
            });
          }
  ...
  const handleLocationSelected = useCallback((address: string, locationData?: LocationData) => {
    if (locationData) {
      ...
      try {
        ...
        console.warn('[BookingFlow] Failed to persist location data:', error);
      }
      console.log('[BookingFlow] Location selected with coordinates:', locationData);
    }
  }, []);
  ...
  if (isLoadingEdit) {
    return (
      <Dashboard userRole="event-host" activeTab='event-host'>
                    ...
                    for (const service of selectedServices) {
                      ...
                      if (serviceType === "catering") {
                        ...
                        if (minimumOrderAmount > 0 && details) {
                          ...
                          console.log('[BookingFlow] Minimum order amount validation:', {
                            serviceName,
                            serviceTotal,
                            minimumOrderAmount,
                            guestCount,
                    ...
                    try {
                      ...
                      const mapServiceWithItems = (service: any, index: number) => {
                        ...
                        if (serviceType === "catering" && service.comboSelectionsList && Array.isArray(service.comboSelectionsList)) {
                          ...
                          service.comboSelectionsList.forEach((combo: any) => {
                            if (combo) {
                              ...
                              if (import.meta.env.DEV) {
                                console.log('[BookingFlow] Combo pricing calculation:', {
                        ...
                        if (normalizedServiceType === "catering") {
                          ...
                          if (import.meta.env.DEV) {
                            console.log('[BookingFlow] Delivery ranges for service:', {
                              serviceName: service.serviceName || service.name,
                              deliveryOptions,
                          ...
                          if (service.service_details) {
                            ...
                            if (import.meta.env.DEV) {
                              console.log('[BookingFlow] Catering service total calculation:', {
                                serviceName: service.serviceName || service.name,
                                basePricePerPerson,
                                guestCount,
                     ...
                     //  console.log("<<<<<<<<<<<<<<<<<<<<<PAYLOAD>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>",JSON.stringify(invoiceData))
                      ...
                      try {
                        ...
                        console.warn('[DeliveryFee] Failed to clear localStorage:', error);
                      }
