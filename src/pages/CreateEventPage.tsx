...
const CreateEventPage = () => {
  ...
  const handleCreateEvent = async (eventData: any) => {
    ...
    try {
      ...
      if (!eventData.startTime && !eventData.startDate.includes('T')) {
        console.error('CreateEventPage: Missing start time', { startDate: eventData.startDate, startTime: eventData.startTime });
        toast.error("Please provide a start time for your event");
        return;
      }
      
      if (!eventData.endTime && !eventData.endDate.includes('T')) {
        console.error('CreateEventPage: Missing end time', { endDate: eventData.endDate, endTime: eventData.endTime });
      ...
      const formattedData = {
        ...
        website: eventData.eventUrl && eventData.eventUrl.trim() ? eventData.eventUrl.trim() : undefined,
        ...
        facebookUrl: eventData.socialMedia?.facebook && eventData.socialMedia.facebook.trim() ? eventData.socialMedia.facebook.trim() : undefined,
        twitterUrl: eventData.socialMedia?.twitter && eventData.socialMedia.twitter.trim() ? eventData.socialMedia.twitter.trim() : undefined,
        instagramUrl: eventData.socialMedia?.instagram && eventData.socialMedia.instagram.trim() ? eventData.socialMedia.instagram.trim() : undefined,
        linkedInUrl: eventData.socialMedia?.linkedin && eventData.socialMedia.linkedin.trim() ? eventData.socialMedia.linkedin.trim() : undefined,
        ...(eventData.isTicketed === true && eventData.ticketTypes?.length > 0 && {
      ...
      console.log("Creating event with final data:", JSON.stringify(formattedData, null, 2));
      ...
      toast.success("Event created successfully", {
        description: "Your event has been created and is ready to share with guests."
      });
      ...
      if (error && typeof error === 'object' && 'response' in error) {
        ...
        if (error.message.includes('Invalid') && error.message.includes('date/time')) {
          errorMessages = ["Invalid date or time format. Please check your event dates and times."];
        } else if (error.message.includes('authentication')) {
          errorMessages = ["Authentication error. Please log in and try again."];
        } else if (error.message.includes('permission') || error.message.includes('policy')) {
          errorMessages = ["Permission denied. Please check your account permissions."];
  ...
  const validateAddressData = (eventData: any) => {
    const requiredFields = ['addressStreet', 'addressCity', 'addressState', 'addressZip'];
    ...
    <Dashboard activeTab="analytics" userRole="event-host">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">Set up your event details and invite your guests</p>
        </div>
