...
} from "@/utils/order-summary-utils";
...
const OrderItemsBreakdown = ({
  ...
  const effectiveSettings = serviceFeeOverride ||
    settings || {
  ...
  const _originalServiceFee =
    pricingSnapshot?.serviceFee ?? calculatedTotals?.serviceFee ?? 0;
  ...
  const apiDeliveryResult = useApiDeliveryCalculations(
    ...
    subtotal,
    distancesByService?.[primaryService?.id || primaryService?.serviceId || ""]
      ?.distanceMiles,
  ...
  const adjustmentsTotal =
    pricingSnapshot?.adjustmentsTotal ??
    calculatedTotals?.adjustmentsTotal ??
    0;
  const adjustmentsBreakdown =
    pricingSnapshot?.adjustmentsBreakdown ??
    calculatedTotals?.adjustmentsBreakdown ??
    [];
  ...
  const cateringCalculations = useMemo(() => {
    const calculations: Record<
      ...
      ReturnType<typeof calculateCateringPrice> & {
        ...
        // Keep flat lists for backward compat with total calculations
        simpleItems: Array<{
          name: string;
          quantity: number;
          price: number;
    ...
    services.forEach((service) => {
      ...
      if (serviceType === "catering" && service.service_details) {
        ...
        let additionalChargeItems: Array<{
          id: string;
          name: string;
          price: number;
          additionalCharge: number;
        ...
        if (apiComboCategoryItems.length > 0 || apiCateringItems.length > 0) {
          ...
          comboCategoryItems = apiComboCategoryItems.map((item: any) => ({
            ...
            additionalCharge:
              ...
              (item.premiumCharge ? parseFloat(String(item.premiumCharge)) : 0),
            image: item.image || item.imageUrl || "",
            comboId: item.comboId || "",
          }));

          // Collect all comboIds to identify which cateringItems are combo base items
          const comboIds = new Set(
            apiComboCategoryItems
              .map((item: any) => item.comboId)
              .filter(Boolean),
          );

          // Separate cateringItems into combo base items vs pure menu items
          apiCateringItems.forEach((item: any) => {
            ...
            if (isComboBase) {
              ...
              comboBaseItems[itemId] = baseData;
              if (cateringId && cateringId !== itemId) {
                comboBaseItems[cateringId] = baseData;
              }
            } else if (quantity > 0) {
              // Pure menu item
              const price =
          ...
          const extracted = extractCateringItems(
            selectedItems,
            service.service_details,
          );
          baseItems = extracted.baseItems;
          additionalChargeItems = extracted.additionalChargeItems.map(
            (item) => ({
              ...item,
        ...
        // Group combo category items by comboId
        const comboGroups: Record<string, typeof comboCategoryItems> = {};
        comboCategoryItems.forEach((item) => {
          const cid = item.comboId || "_default";
          if (!comboGroups[cid]) comboGroups[cid] = [];
          comboGroups[cid].push(item);
        });

        // Build per-combo data
        const combos = Object.entries(comboGroups).map(([comboId, items]) => {
          const baseItem = comboBaseItems[comboId];
          return {
            comboId,
            comboName: baseItem?.name || "Combo",
        ...
        // Flat lists (for total calculations)
        const simpleItems = comboCategoryItems
          .filter(
            (item) => !item.additionalCharge || item.additionalCharge === 0,
          )
          .map((item) => ({
  ...
  return (
    <Card className="bg-white border border-gray-200 shadow-sm">
      ...
      <CardContent className="space-y-5 pt-5">
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100">
            <div className="space-y-4">
              ...
              {services.map((service, serviceIndex) => {
                ...
                const servicePrice =
                  parseFloat(formattedServicePrice.replace(/[^0-9.]/g, "")) ||
                  0;
                const menuItems = getMenuItems(service);
                const hasMenuItems = menuItems && menuItems.length > 0;
                const requiresSelection = requiresItemSelection(
                ...
                if (service.serviceType === "staff") {
                  ...
                  if (staffRoleKeys.length > 0) {
                    totalStaffCount = staffRoleKeys.reduce(
                      (sum, key) => sum + (selectedItems[key] || 0),
                      0,
                    );
                  }
                  ...
                  if (totalStaffCount > 0) {
                    ...
                    staffCount =
                      selectedItems[serviceId] || service.quantity || 1;
                  }
                  ...
                  if (durations.length > 0) {
                    ...
                    effectiveDuration =
                      selectedItems[`${serviceId}_duration`] ||
                      service.duration ||
                      service.service_details?.staff?.minimumHours ||
                      1;
                  }
                          ...
                          cateringCalc.menuItems.length > 0 && (
                            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                              ...
                              {cateringCalc.menuItems.map((item, idx) => (
                                <div
                                  key={idx}
                                  className="grid grid-cols-12 gap-2 py-2 border-b border-gray-100 last:border-0 items-center"
                                >
                                  ...
                                  <div className="col-span-3 text-right">
                                    <span className="text-gray-700 text-sm">
                                      {formatCurrency(item.additionalCharge)}
                                    </span>
                                  </div>
                                  <div className="col-span-2 text-center">
                                    <span className="text-gray-700 text-sm">
                                  ...
                                  <div className="col-span-2 text-right">
                                    <span className="text-gray-900 font-semibold text-sm">
                                      {formatCurrency(
                                        item.additionalCharge * item.quantity,
                                      )}
                                    </span>
                                  </div>
                                </div>
                    ...
                    {service.serviceType === "staff" &&
                      (() => {
                        ...
                        const staffItemsList =
                          details?.staffServices || details?.services || [];
                ...
                return (
                  ...
                  >
                    <span className="text-gray-600">
                      {adjustment.label}{" "}
                      {isPercentage && adjustment.value
                        ? `(${adjustment.value}%)`
                        : ""}
                    </span>
                    <span
                      className={`font-semibold ${isSurcharge ? "text-gray-900" : "text-green-600"}`}
                    >
                      {isSurcharge ? "+" : "-"}
        ...
        {/* Delivery Fees Section (per vendor) - before subtotal */}
        {(() => {
          const serviceDeliveryFees = services
            .filter((s) => {
              const fee =
                parseFloat(String((s as any).deliveryFee || "0")) || 0;
              return fee > 0;
            })
            .map((s) => ({
          ...
          const totalServiceDeliveryFees = serviceDeliveryFees.reduce(
                ...
                {serviceDeliveryFees.length > 1 && (
                  <>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg font-bold text-gray-900">
                        {formatCurrency(totalServiceDeliveryFees)}
                      </span>
                    </div>
                  </>
                )}
        ...
        <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100">
          <div className="flex justify-between items-start">
            ...
            <span className="text-xl font-bold text-gray-900">
              {formatCurrency(subtotal + deliveryFee + adjustmentsTotal)}
            </span>
          </div>
        </div>
        ...
        {(serviceFee > 0 || isServiceFeeWaived) && (
          <div className="bg-white rounded-lg p-4 space-y-2 shadow-sm border border-gray-100">
            <div className="flex justify-between items-center text-sm">
              ...
              <span
                className={`font-semibold ${isServiceFeeWaived ? "line-through text-gray-400" : "text-gray-900"}`}
              >
        ...
        {(() => {
          ...
          const taxData = pricingSnapshot
            ? {
                rate: pricingSnapshot.taxRate,
                description: `Tax (${(pricingSnapshot.taxRate * 100).toFixed(3)}%)`,
                jurisdiction: formData?.location,
              }
            : calculatedTotals?.taxData;
          const taxRate = taxData?.rate ?? 0;
          const taxAmount = recalculatedTax;
        ...
        <div className="bg-white rounded-lg p-5 shadow-sm border border-gray-200">
          <div className="flex justify-between items-center">
            ...
            <span className="text-3xl font-bold text-gray-900">
              {formatCurrency(serviceFeeBase + serviceFee + recalculatedTax)}
            </span>
          </div>
        </div>
      </CardContent>
