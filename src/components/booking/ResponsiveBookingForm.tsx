...
function ResponsiveBookingForm({
  ...
  const { checkMultipleVendorsAvailability, getBlockedDates, getClosedDays } = useVendorAvailabilityCheck();
  ...
  const { isSaving } = useFormAutoSave({
    formData,
    onSave: async (data) => {
      // Save to booking state persistence
  ...
  // Handle field focus for validation
  const handleFieldFocus = (fieldName: string) => {
    setTouchedFields(prev => ({ ...prev, [fieldName]: true }));
    touchField(fieldName);
  };
  
  // Load blocked dates and closed days when services change
  useEffect(() => {
    if (!selectedServices || selectedServices.length === 0) return;
    
    const loadAvailabilityRestrictions = async () => {
      const vendorIds = [...new Set(selectedServices.map((s: any) => s.vendor_id).filter(Boolean))];
  ...
  // Extract catering services that need service style selection
  const cateringServices = selectedServices.filter(service => 
    (service.serviceType === 'catering' || service.type === 'catering')
  );

  // Get available service styles for each catering service
  const getCateringServiceStyles = (service: any): CateringServiceStyle[] => {
    // Check different possible locations for service styles
    ...
    <div className="w-full max-w-full overflow-x-hidden">
      <Card className="bg-card shadow-sm border">
        ...
        <CardContent className="space-y-4 px-3 sm:px-6 pb-3 sm:pb-6">
          {/* Invoice-specific Client Information */}
          {isInvoiceMode && (
            <>
              <div className="space-y-3">
                ...
                <div>
                  <Label htmlFor="invoiceExpiryDate" className="text-sm font-medium text-gray-700">
                    Invoice Expiry Date
                  </Label>
                  <Input
                ...
                <div>
                  <Label htmlFor="invoiceMessage" className="text-sm font-medium text-gray-700">
                    Invoice Message
                  </Label>
                  <Textarea
                    id="invoiceMessage"
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <Label htmlFor="orderName" className="text-sm font-medium text-gray-700">
                  Event Name *
                </Label>
                <Input
                  id="orderName"
              ...
              {/* Company Name Field - Only for regular bookings */}
              {!isInvoiceMode && (
                <div>
                  ...
                  <DatePicker
                    ...
                    onSelect={async (selectedDate) => {
                      if (selectedDate) {
                        ...
                        // Check if any vendor is unavailable on this date
                        const vendorIds = [...new Set(selectedServices.map((s: any) => s.vendor_id).filter(Boolean))];
                        ...
                        if (vendorIds.length > 0) {
                          ...
                          if (!allAvailable) {
                            // Show specific error message with vendor names
                    ...
                    disabled={(date) => {
                      // Disable past dates (except for admins)
                      if (!isAdmin) {
                      ...
                      // Disable dates that are blocked for any vendor
                      ...
                      // Disable days when any vendor is closed
                ...
                <div className="mt-1">
                  <TimePicker
                    ...
                    onSelect={(selectedTime) => {
                      onChange({
                        target: { name: 'deliveryWindow', value: selectedTime || '' }
                      } as React.ChangeEvent<HTMLInputElement>);
                    }}
          ...
          {/* Contact Information - only show if not in invoice mode */}
          {!isInvoiceMode && (
            <>
              <Separator />
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) => onChange({
                          target: { name: 'hasBackupContact', value: checked, type: 'checkbox' }
                        } as any)}
                      />
