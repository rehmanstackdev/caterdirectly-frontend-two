...
export const BookingOrderSummary = ({
  ...
  const deliveryService = selectedServices.find(
    (service) =>
      service.service_details?.deliveryOptions?.delivery ||
      service.service_details?.catering?.deliveryOptions?.delivery,
  );
  const vendorOriginAddress =
    (deliveryService as any)?.location ||
    (deliveryService as any)?.service_details?.location ||
    "";

  const { distanceMiles } = useAddressDistance(
    vendorOriginAddress,
    deliveryAddress,
  );
  ...
  const deliveryOptions =
    deliveryService?.service_details?.deliveryOptions ||
  ...
  return (
    <div className={`space-y-4 ${className}`}>
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500">
                            Custom{" "}
                            {a.mode === "discount" ? "Discount" : "Surcharge"} •{" "}
                            {signBadge}
                            {valueText}
                            {a.taxable === false ? " • non-taxable" : ""}
                          </p>
                        </div>
      ...
      {calculations.deliveryDetails &&
        !calculations.deliveryDetails.eligible && (
