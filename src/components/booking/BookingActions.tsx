...
function BookingActions({
  ...
  const getButtonIcon = () => {
    if (isLoading)
      return (
        <Loader2 className="mr-2 h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0 animate-spin" />
      );
    if (isInvoiceMode)
      return <FileText className="mr-2 h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0" />;
  ...
  return (
    <div className="bg-white border-t border-gray-200 pt-6 mt-8 w-full max-w-full overflow-x-hidden">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 lg:gap-6 w-full max-w-full overflow-x-hidden">
        ...
        {/* Action buttons */}
        <div className="flex flex-wrap items-center justify-between sm:justify-end gap-3 lg:gap-4 w-full lg:w-auto lg:flex-shrink-0 overflow-x-hidden">
          <Button
            variant="outline"
            onClick={() => navigate("/marketplace")}
            className="w-auto min-h-[44px] sm:min-h-[48px] text-sm sm:text-base font-medium border-gray-300 hover:bg-gray-50 max-w-full overflow-hidden"
          >
          ...
          <Button
            onClick={onSubmit}
            disabled={isLoading}
            className={`${getButtonColor()} w-auto min-h-[44px] sm:min-h-[48px] px-3 sm:px-4 md:px-6 lg:px-8 text-sm sm:text-base font-semibold text-white shadow-lg hover:shadow-xl transition-all duration-200 max-w-full overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed`}
          >
            {getButtonIcon()}
            <span className="truncate max-w-full">{getButtonText()}</span>
          </Button>
        </div>
      </div>
    </div>
