...
const CateringTab = ({
  ...
  // Get location coordinates from global filters
  ...
  const apiFilters = useMemo(() => {
    ...
    return {
      ...
      // Only use text-based location filter if no coordinates (manual text entry)
  ...
  const {
    ...
    activeTab: SERVICE_TYPES.CATERING,
  ...
  return (
    <div className="space-y-6">
      <ServiceSection
        ...
        serviceType={SERVICE_TYPES.CATERING}
        isLoading={isLoading}
        error={error}
      ...
      <MarketplacePagination
        ...
        totalPages={pagination?.totalPages || Math.ceil(filteredServices.length / itemsPerPage) || 1}
        totalItems={pagination?.totalItems || filteredServices.length}
